...
PAnalysis 是一个独立桌面抓包分析软件，当前阶段重点聚焦 `SIP`、`RTP` 和 `RTCP`，用于离线分析 `pcap/pcapng` 抓包文件，不做实时抓包。纯 AI 制作，莫得感情。

功能目前还没有做系统化测试，后续会结合真实 SIP 问题继续补样本和验证。
...
│  └─ preload.ts           # 旧版 preload，当前不作为运行入口
├─ shared/                 # 主进程 / 渲染进程共享类型
├─ src/
│  ├─ app/                 # React 页面和样式
│  └─ main.tsx             # 前端入口
...
- `pnpm dev` 启动后，真正可用的是弹出的 Electron 桌面窗口
- `http://localhost:5173` 只是前端预览页，不带抓包文件选择和本地 IPC 能力
